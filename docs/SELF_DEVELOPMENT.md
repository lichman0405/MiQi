...
miqi self-development is implemented as a lightweight feedback loop:
...
When a tool returns an error result, miqi learns a tool-specific lesson, for example:
...
At context build time, miqi selects top lessons by:
...
To improve reuse without making the system heavy, miqi can auto-promote repeated
