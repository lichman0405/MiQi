...
def mock_prompt_session():
    ...
    with patch("miqi.cli.commands._PROMPT_SESSION", mock_session), \
         patch("miqi.cli.commands.patch_stdout"):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    with (
        patch("miqi.cli.commands.PromptSession") as mock_session_cls,
        patch("miqi.cli.commands.FileHistory"),
...
def test_fetch_ollama_cloud_models_from_tags() -> None:
    ...
    with patch("miqi.cli.commands.httpx.get", return_value=response) as mock_get:
...
def test_fetch_ollama_cloud_models_fallback_to_v1_models() -> None:
    ...
    with patch("miqi.cli.commands.httpx.get", side_effect=[first_response, second_response]) as mock_get:
        models, error = commands._fetch_ollama_cloud_models(
